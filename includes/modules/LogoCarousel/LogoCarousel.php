	...
	function render( $attrs, $content, $render_slug ) {
	   ...
	   // Navigation Background Color 
		...
		$output = sprintf(
			...
			/* 05 */ esc_attr( $autoplay_value ),
			/* 06 */ esc_attr( $autoplay_speed ),
			/* 07 */ esc_attr( $navigation_value ),
			/* 08 */ esc_attr( $pagination_value ),
			/* 09 */ esc_attr( $infinite_value ),
			/* 10 */ esc_attr( $pause_on_hover_value ),
			/* 11 */ esc_attr( $swipe_value ),
			/* 12 */ esc_attr( $rtl_value ),
			/* 13 */ esc_attr( $dir_value ),
			/* 14 */ esc_attr( $slides_to_show_tablet ),
			/* 15 */ esc_attr( $slides_to_show_phone ),
			/* 16 */ esc_attr( $slides_to_scroll_tablet ),
			/* 17 */ esc_attr( $slides_to_scroll_phone )	
